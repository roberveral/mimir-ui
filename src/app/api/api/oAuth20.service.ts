...
export class OAuth20Service {
     ...
     * This endpoint performs the first step in the OAuth 2 Authorization Code flow. When a call to this endpoint is performed by an authenticated user, it means that the user is authorizing the client defined by the client_id to act on his behalf and therefore to obtain an access token. The frontend MUST redirect the user to the returned redirect_uri in order to continue with the authorization flow. 
     ...
     */
    public authorizeOAuthClient(responseType: 'code', clientId: string, redirectUri?: string, scope?: string, state?: string, observe?: 'body', reportProgress?: boolean): Observable<OAuthAuthorizeResponse>;
    public authorizeOAuthClient(responseType: 'code', clientId: string, redirectUri?: string, scope?: string, state?: string, observe?: 'response', reportProgress?: boolean): Observable<HttpResponse<OAuthAuthorizeResponse>>;
    public authorizeOAuthClient(responseType: 'code', clientId: string, redirectUri?: string, scope?: string, state?: string, observe?: 'events', reportProgress?: boolean): Observable<HttpEvent<OAuthAuthorizeResponse>>;
    public authorizeOAuthClient(responseType: 'code', clientId: string, redirectUri?: string, scope?: string, state?: string, observe: any = 'body', reportProgress: boolean = false ): Observable<any> {
     ...
     * This endpoint allows a client to request an access token in order to access protected resources. Depending on the grant_type, this token can be to act on behalf of a given user or by the client itself. The access token is a JWT token signed with the public key of the authorization server and which contain the following information.   - sub (username or client_id, depending on the entity which holds the permission).   - cid (client who acts on behalf of the subject).   - name (name of the authenticated user).   - email (email of the authenticated user).   - picture (url to the user profile picture).    Resource servers can verify the token integrity and expiration and get the requesting user from this token. 
     ...
     * @param clientSecret The client’s secret. This ensures that the request to get the access token is made only from the client. (Authorization Code and Client Credentials Flows - REQUIRED)
     ...
     * @param reportProgress flag to report request and response progress.
     */
    public getOAuthToken(grantType: string, clientId: string, code?: string, redirectUri?: string, clientSecret?: string, username?: string, password?: string, scope?: string, observe?: 'body', reportProgress?: boolean): Observable<OAuthTokenResponse>;
    public getOAuthToken(grantType: string, clientId: string, code?: string, redirectUri?: string, clientSecret?: string, username?: string, password?: string, scope?: string, observe?: 'response', reportProgress?: boolean): Observable<HttpResponse<OAuthTokenResponse>>;
    public getOAuthToken(grantType: string, clientId: string, code?: string, redirectUri?: string, clientSecret?: string, username?: string, password?: string, scope?: string, observe?: 'events', reportProgress?: boolean): Observable<HttpEvent<OAuthTokenResponse>>;
    public getOAuthToken(grantType: string, clientId: string, code?: string, redirectUri?: string, clientSecret?: string, username?: string, password?: string, scope?: string, observe: any = 'body', reportProgress: boolean = false ): Observable<any> {
